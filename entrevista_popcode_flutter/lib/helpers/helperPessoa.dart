...
class HelperPessoa extends HelperBase<Pessoa> {
  ...
  Future<Pessoa> save(Pessoa pessoa) async {
    ...
    Pessoa p = await getFirst(pessoa.name);
    if(p == null)
  ...
  Future<Pessoa> getFirst(String nome) async {
    Database database = await db;
    List<Map> personagem = await database.rawQuery("SELECT * FROM $pessoaTable WHERE $nameColumn LIKE '$nome'");
  ...
  Future<int> delete(String nome) async {
    ...
    return await database
        .delete(pessoaTable, where: "$nameColumn = ?", whereArgs: [nome]);
  }
